...
object RetrofitClient {
    val retrofit =
        Retrofit.Builder().baseUrl("http://www.jsonplaceholder.typicode.com").addConverterFactory(
            GsonConverterFactory.create()
        ).build()
    val apiInterface= retrofit.create(PostInterface::class.java)

    fun getPosts(onDataRecieved:(ArrayList<PostItem>) ->Unit ):ArrayList<PostItem>{
